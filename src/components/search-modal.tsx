...
import { streamLLMAgent, StreamCallbacks, BOT_ID_YIXIN } from '@/lib/coze-api';
...
export function SearchModal({ isOpen, onClose }: SearchModalProps) {
  ...
  const handleSend = useCallback(async () => {
    ...
    await streamLLMAgent(query, userId, conversationIdRef.current || undefined, chatHistory, callbacks, BOT_ID_YIXIN);
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            {/* 头部 */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 flex-shrink-0 relative">
              {/* 移动端下拉提示 */}
              ...
              <div className="flex items-center gap-2 mt-2 md:mt-0">
                <div className="w-6 h-6 bg-white rounded-full flex items-center justify-center">
                  <span className="text-black text-xs font-medium">意</span>
                </div>
                <span className="text-white font-medium">意心之镜</span>
              </div>
              <button
                onClick={onClose}
              ...
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center px-4">
                  <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center mb-4">
                    <span className="text-black text-lg font-medium">意</span>
                  </div>
                  <h2 className="text-white text-lg font-medium mb-2">意心之镜</h2>
                  <p className="text-gray-500 text-center text-sm mb-3">体验 AI 能力，和意心之境聊聊吧</p>
                  <div className="max-w-md text-gray-400 text-xs leading-relaxed text-left w-full px-4">
                    <p className="mb-1">我是「意心之镜」，一个陪你理清内心卡点、看清自己真实想法的对话伙伴。</p>
                    <p className="mb-1">我不给标准答案，但擅长通过提问和对话，帮你打破犹豫、找到属于自己的下一步。</p>
                    <p>如果你正卡在某个人生选择、工作决定或情感困惑里，我们可以聊聊，看看你究竟在怕什么、在等什么。</p>
                  </div>
                </div>
              ) : (
